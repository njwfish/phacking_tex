...
Write \emph{Main} for the first document, \emph{Unified} for the
second, and \emph{Technical} for
\emph{Technical Proofs for Unified Moment Approximation}.  Result and
equation numbers refer to their current numbered versions.  The scope
...
Corollary~2.3, its coefficient specialization.  Technical
Lemmas~2.1--3.1 and Theorem~3.2 contain the shared proof engine.  The
table below identifies the endpoint used for each Main claim.
...
(88)--(91), gives the score-packing lower bound.  Technical
Lemmas~4.2--4.3 give the score-to-direction bridge. &
...
Technical Lemmas~2.1--3.1 and Theorem~3.2. \\
\addlinespace
...
$O_{\Prob}(L/n)$.  The exact normalized-residual identity in Technical
Lemma~4.1 similarly gives $O_{\Prob}(1+L/\sqrt n)=O_{\Prob}(1)$
