...
fourth-moment dependence.  Gaussian multipliers give the corresponding
conditional approximation from the row scores.  Conditional-product
greedy maximizes the exact Schur-complement increments and becomes top-$s$
sorting when the controls are uncorrelated.  The safeguarded search has vanishing error on
the search scale under local isotropy; for aligned scores, restricted
conditioning gives a constant-factor guarantee.
\end{abstract}
...
\end{equation*}
If $\Delta=\widehat\Sigma_U-\Sigma_U$, differentiating the interpolated
...
\begin{equation}\label{eq:greedy-rule}
        ...
        {c_{j\mid S_{t-1}}}\right\},
        \qquad
        S_t=S_{t-1}\cup\{j_t\}.
\end{equation}
The increments telescope, and rank-one updates of the conditional
variance and scores give a polynomial-time implementation.  When $C=I$,
the rule selects the largest $s$ products $-a_jb_j$.
...
\end{equation}
The local-isotropy coefficient is
\begin{equation}\label{eq:local-isotropy}
        \rho_s(C)
        =\sup_{|J|\le2s}\norm{C_{JJ}-I}_{\mathrm{op}}.
\end{equation}

\begin{theorem}[Greedy search under local isotropy]
\label{thm:conditional-search}
Suppose $\rho=\rho_s(C)\le1/2$, and let $S^\circ$ contain the largest $s$
singleton scores $w_j=-a_jb_j$.  Let $\widehat S$ be whichever of
$S^\circ$ and $S_s$ has the larger value of $Q_S(a,b)$.  Then
\begin{equation}\label{eq:safeguarded-regret}
        0\le T_C(a,b)-Q_{\widehat S}(a,b)
        \le\frac{2\rho}{1-\rho}
        \norm{a}_{2,s}\norm{b}_{2,s}.
\end{equation}
The greedy support itself satisfies
\begin{equation}\label{eq:greedy-regret}
\begin{split}
        0\le T_C(a,b)-Q_{S_s}(a,b)
        \lesssim{}&
        \rho\norm{a}_{2,s}\norm{b}_{2,s}\\
        &+s\rho\left(
        \norm{a}_\infty\norm{b}_{2,s}
...
\begin{proof}
Restricted inverse perturbation gives, uniformly over $|S|\le s$,
\begin{equation}\label{eq:objective-error}
        \left|Q_S(a,b)-\sum_{j\in S}w_j\right|
        \le\frac{\rho}{1-\rho}
        \norm{a}_{2,s}\norm{b}_{2,s}.
\end{equation}
The largest sum of $s$ singleton scores is attained at $S^\circ$.
Applying \eqref{eq:objective-error} to $S^\circ$ and to an optimal support
proves \eqref{eq:safeguarded-regret}.

For $j\notin S$, local isotropy gives
\begin{equation*}
        |a_j-a_{j\mid S}|
        \le\frac{\rho}{1-\rho}\norm{a}_{2,s},
        \qquad
        ...
        \le\frac{\rho}{1-\rho}\norm{b}_{2,s},
        \qquad
        0\le1-c_{j\mid S}\le\frac{\rho^2}{1-\rho}.
\end{equation*}
Together with \eqref{eq:conditional-increment}, these bounds imply
\begin{equation}\label{eq:marginal-error}
\begin{split}
        \sup_{\substack{|S|<s\\j\notin S}}
        \left|Q_{S\cup\{j\}}(a,b)-Q_S(a,b)-w_j\right|
        \lesssim{}&
        \rho\left(
        \norm{a}_\infty\norm{b}_{2,s}
        +\norm{b}_\infty\norm{a}_{2,s}
...
Let $w_{(t)}$ be the $t$th largest singleton score.  At iteration $t$, one
of the $t$ largest scores remains unselected, so the greedy rule and
\eqref{eq:marginal-error} put $w_{j_t}$ at most twice the right-hand side
of \eqref{eq:marginal-error} below $w_{(t)}$.  Summing over $t\le s$ and
applying \eqref{eq:objective-error} to the greedy and optimal supports
proves \eqref{eq:greedy-regret}.
\end{proof}

\begin{corollary}[Stochastic greedy approximation]
\label{cor:stochastic-search}
Suppose $\rho_s(C)=o(1)$ and either the assumptions of
Theorem~\ref{thm:matched-chaos} hold with $(sL)^4=o(n)$ or the jointly
Gaussian assumptions of Corollary~\ref{cor:gaussian} hold with
$sL=o(n)$.  Let $\widehat S$ be the safeguarded support computed from
$(Z_1,Z_2)$.  Then
\begin{equation}\label{eq:algorithmic-chaos}
        T_C(Z_1,Z_2)-Q_{\widehat S}(Z_1,Z_2)
        =o_{\Prob}(\sigma_0\sigma_rsL).
\end{equation}
Under the coupling in Theorem~\ref{thm:matched-chaos} or
Corollary~\ref{cor:gaussian},
\begin{equation}\label{eq:algorithmic-approximator}
        \sup_{|S|=s}\widehat\beta_{0,S}
        =\widehat\beta_{0,\emptyset}
        +\frac{Q_{\widehat S}(Z_1,Z_2)}{n\sigma_0^2}
        +o_{\Prob}\!\left(
        \frac{\sigma_r}{\sigma_0}\frac{sL}{n}
        \right).
\end{equation}
An independent draw of $Z$ also gives
...
\begin{split}
        ...
        -\widehat\beta_{0,\emptyset}
        \right\},
        \frac{Q_{\widehat S}(Z_1,Z_2)}
        {\sigma_0\sigma_rsL}
        \right)=o(1).
\end{split}
\end{equation}
Whenever Corollary~\ref{cor:multiplier} applies, the conditional version
of \eqref{eq:algorithmic-distribution} holds with
$Q_{\widehat S^\#}(Z_1^\#,Z_2^\#)$, where $\widehat S^\#$ is the
safeguarded support computed from $Z^\#$.

The safeguard can be omitted if, in addition,
\begin{equation}\label{eq:coordinate-proxies}
\begin{split}
        \max_{j\le p}\frac{|d_j|}{\sigma_0}
        &\lesssim\sqrt{\frac{L}{n}},\\
        \max_{j\le p}\frac{|h_j|}{\sigma_r}
        &\lesssim\sqrt{\frac{L}{n}},
\end{split}
\end{equation}
and
\begin{equation}\label{eq:greedy-rate}
        \rho_s(C)
        \sqrt{\frac{s\log(ep)}{L}}\longrightarrow0.
\end{equation}
Under \eqref{eq:greedy-rate}, \eqref{eq:algorithmic-chaos} holds with
$S_s$ in place of $\widehat S$.
\end{corollary}

\begin{proof}
Lemma~\ref{lem:sparse-score-moments} and local isotropy give
\begin{equation*}
\begin{aligned}
        \E\norm{Z_1}_{2,s}^2
        &\lesssim\sigma_0^2sL,
...
\end{aligned}
\end{equation*}
Hence the expected value of the normalized regret in
\eqref{eq:safeguarded-regret} is $O(\rho_s(C))$.  This proves
\eqref{eq:algorithmic-chaos}.  Combining the regret bound with
Theorem~\ref{thm:matched-chaos} or Corollary~\ref{cor:gaussian} proves
\eqref{eq:algorithmic-approximator} and
\eqref{eq:algorithmic-distribution}.

By Markov's inequality, the moment bounds above also give
$\norm{Z_1}_{2,s}=O_{\Prob}(\sigma_0\sqrt{sL})$ and
$\norm{Z_2}_{2,s}=O_{\Prob}(\sigma_r\sqrt{sL})$.  If
\eqref{eq:coordinate-proxies} holds, then
\begin{equation*}
        \norm{Z_1}_\infty
...
\end{equation*}
Substituting the four score bounds into \eqref{eq:greedy-regret} proves
\eqref{eq:algorithmic-chaos} with $S_s$ in place of $\widehat S$.

For the multiplier version of \eqref{eq:algorithmic-distribution}, write
$\E^\#$ for expectation over the multipliers conditional on the data.
Lemma~\ref{lem:multiplier-covariance} and a conditional Gaussian net
argument give
\begin{equation*}
\begin{aligned}
        \E^\#\norm{Z_1^\#}_{2,s}^2
...
\end{aligned}
\end{equation*}
The safeguard therefore satisfies
\begin{equation*}
        \E^\#\!\left[
        \frac{T_C(Z_1^\#,Z_2^\#)
        -Q_{\widehat S^\#}(Z_1^\#,Z_2^\#)}
        {\sigma_0\sigma_rsL}
        \right]
        =O_{\Prob}(\rho_s(C))=o_{\Prob}(1).
\end{equation*}
Together with Corollary~\ref{cor:multiplier}, this proves the conditional
version of \eqref{eq:algorithmic-distribution}.
\end{proof}

\section{Aligned scores}

When $b=-a$, restricted conditioning yields a multiplicative guarantee.

\begin{proposition}[Restricted conditioning]
\label{prop:aligned}
Suppose $b=-a$ and
\begin{equation*}
        \kappa_s(C)
        =\sup_{|J|\le2s}
        \frac{\lambda_{\max}(C_{JJ})}
        {\lambda_{\min}(C_{JJ})}
        \le K.
\end{equation*}
...
\begin{proof}
$Q_S(a,-a)=a_S^\top C_{SS}^{-1}a_S$ is nonnegative and monotone.  For
disjoint $A,S$, let $a_{S\mid A}$ and $C_{S\mid A}$ denote the conditional
score and Schur complement, and put $J=A\cup S$.
The eigenvalues of the Schur complement lie between the extreme
eigenvalues of $C_{JJ}$, so
\begin{equation*}
\begin{split}
        Q_{A\cup S}(a,-a)-Q_A(a,-a)
        &=a_{S\mid A}^\top C_{S\mid A}^{-1}a_{S\mid A}
        \le\frac{\norm{a_{S\mid A}}_2^2}
        {\lambda_{\min}(C_{JJ})},\\
        \sum_{j\in S}\{Q_{A\cup\{j\}}(a,-a)-Q_A(a,-a)\}
        &\ge\frac{\norm{a_{S\mid A}}_2^2}
        {\lambda_{\max}(C_{JJ})}.
\end{split}
\end{equation*}
...
For general $a,b$, polarization gives
\begin{equation}\label{eq:polarization}
        ...
        \right\}.
\end{equation}
Thus the support value is a difference of two monotone quadratic set
functions.  Even arbitrarily small correlations allow greedy search to
fail.  For $s=2$ and $0<\tau<1/\sqrt2$, take
\begin{equation*}
        C=
        \begin{pmatrix}
        1&\tau&\tau\\
        ...
        b=(-2\tau,-\tau^{-1},-\tau^{-1})^\top.
\end{equation*}
The singleton scores are $2,1,1$, so greedy selects coordinate $1$, while
$Q_{\{2,3\}}(a,b)=2$.  The resulting greedy support has value
\begin{equation*}
        Q_{S_2}(a,b)
        =\frac{3-\tau^{-1}-2\tau^3}{1-\tau^2}
        \longrightarrow-\infty.
\end{equation*}
At the same time, $\rho_s(C)=\sqrt2\tau\to0$.  Thus covariance control
alone gives no relative guarantee; the additive bound
\eqref{eq:greedy-regret} also depends on sparse norms of $a$ and $b$.

\section{Manuscript assumptions}

The scale $\delta_{s,n,p}$ controls the whitened block-Gram event
\eqref{eq:block-gram} and hence the empirical reduction
\eqref{eq:empirical-chaos}.  Replacing $Z_n$ by $Z$ requires
$(sL)^4=o(n)$, while the direct coupling for Gaussian rows requires only
$sL=o(n)$.  The local-isotropy coefficient $\rho_s(C)$ enters the greedy
approximation, and the restricted condition number $\kappa_s(C)$ enters
the aligned guarantee.
...
Because $\Sigma_U$ and $\widehat\Sigma_U$ retain fourth-moment dependence,
vanishing normalized fourth-cumulant blocks in restricted operator norm
give a sufficient condition for a uniform diagonal Gaussian replacement
over size-$s$ supports.  Entrywise convergence is insufficient when $s$
grows.
