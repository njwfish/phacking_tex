...
blocks, its regret is bounded by the aggregate equal-depth variation of
the coordinate marginals.  The regret bound yields a stochastic
approximation whenever this variation is negligible.  Local isotropy
...
\begin{equation}\label{eq:greedy-value}
        \widetilde T_C(a,b):=Q_{S_s}(a,b)
        =\sum_{t=1}^s\Delta_{j_t}(S_{t-1};a,b).
\end{equation}
Given $C$, rank-one updates of \eqref{eq:conditional-scores} compute
$\widetilde T_C$ without enumerating the $\binom ps$ supports.  When
$C=I$, the rule selects the largest $s$ products $-a_jb_j$, so
$\widetilde T_I=T_I$.

The aggregate marginal variation sums, over distinct coordinates, the
range of each marginal across supports at the same depth:
\begin{equation}\label{eq:marginal-variation}
\begin{aligned}
        V_C(a,b)
        ...
        \sum_{t=1}^s
        \left\{
        \max_R\Delta_{j_t}(R;a,b)
        -\min_R\Delta_{j_t}(R;a,b)
        \right\}.
\end{aligned}
\end{equation}
In the $t$th summand, both extrema are over
$R\subseteq[p]\setminus\{j_t\}$ with $|R|=t-1$.  Thus $V_C$ compares
a coordinate only across histories of the same size.

\begin{theorem}[Greedy regret]
...
\begin{equation}\label{eq:empirical-greedy}
        ...
        =\widehat\beta_{0,\emptyset}
        +\frac{\widetilde T_C(Z_{1n},Z_{2n})}{n\sigma_0^2}
        +o_{\Prob}\!\left(
        \frac{\sigma_r}{\sigma_0}\frac{sL}{n}
        \right).
\end{equation}
...
$(sL)^4=o(n)$.  If
\begin{equation}\label{eq:matched-variation}
        V_C(Z_1,Z_2)
        =o_{\Prob}(\sigma_0\sigma_rsL),
\end{equation}
...
\begin{equation}\label{eq:algorithmic-approximator}
        \sup_{|S|=s}\widehat\beta_{0,S}
        =\widehat\beta_{0,\emptyset}
        +\frac{\widetilde T_C(Z_1,Z_2)}{n\sigma_0^2}
        +o_{\Prob}\!\left(
        \frac{\sigma_r}{\sigma_0}\frac{sL}{n}
        \right).
\end{equation}
...
\begin{split}
        ...
        -\widehat\beta_{0,\emptyset}
        \right\},
        \frac{\widetilde T_C(Z_1,Z_2)}{\sigma_0\sigma_rsL}
        \right)=o(1).
\end{split}
\end{equation}
...
\eqref{eq:matched-variation}, with $sL=o(n)$.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:greedy-regret} and
\eqref{eq:matched-variation} show that $T_C(Z_1,Z_2)$ and
...
Under the assumptions of Corollary~\ref{cor:multiplier}, suppose that, in
outer probability,
\begin{equation}\label{eq:multiplier-variation}
        V_C(\widehat Z_1,\widehat Z_2)
        =o_{\Prob_e}(\sigma_0\sigma_rsL).
\end{equation}
Then the conditional law of
\begin{equation*}
        \frac{\widetilde T_C(\widehat Z_1,\widehat Z_2)}
        {\sigma_0\sigma_rsL}
\end{equation*}
has $d_3$ distance $o_{\Prob}(1)$ from the law of the normalized searched
displacement in \eqref{eq:smooth-approximation}.
...
\eqref{eq:multiplier-variation} implies that the normalized greedy
...
\begin{equation}\label{eq:isotropy-variation}
        V_C(a,b)
        \lesssim
        (\rho\sqrt{s}+s\rho^2)
        \norm{a}_{C,s}\norm{b}_{C,s}.
\end{equation}
Consequently, $\rho_s(C)\sqrt{s}\to0$ verifies the variation conditions
...
\end{aligned}
\end{equation*}
Applying the bound at the two extrema in each summand and then summing
proves \eqref{eq:isotropy-variation}.
...
\end{aligned}
\end{equation*}
Together with \eqref{eq:isotropy-variation}, these bounds verify the
variation conditions in Corollaries~\ref{cor:empirical-greedy} and
...
\begin{equation*}
        \norm{\widehat Z_1}_{C,s}
        =O_{\Prob_e}(\sigma_0\sqrt{sL}),
        \qquad
        \norm{\widehat Z_2}_{C,s}
        =O_{\Prob_e}(\sigma_r\sqrt{sL}).
\end{equation*}
Since $s\rho^2=(\rho\sqrt{s})^2$, substitution in
\eqref{eq:isotropy-variation} proves
\eqref{eq:multiplier-variation}.
\end{proof}
...
requires only $sL=o(n)$.  Marginal variation instead controls the
computational reduction from $T_C$ to $\widetilde T_C$.  These are
distinct statistical and computational steps.
Theorem~\ref{thm:greedy-regret} takes marginal variation as its
computational condition, and Corollary~\ref{cor:local-isotropy} bounds that
variation under local isotropy.  The resulting rate
$\rho_s(C)\sqrt{s}\to0$ is stronger than the manuscript's
$\rho_s(C)=o(1)$.  Neither the relative RIP nor the variation condition
implies the other.
